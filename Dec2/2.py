...
# in here we take a prediction and we translate it to the right number
# @prediction is the letter that point out to us the action we need to perform (win lose or draw)
# @p1Coice is the player one choice
# @return our choice depending on the prediction
def ChosingYourNumber(prediction, p1Coice):
    # win
    if (prediction == "Z"):
        if(p1Coice == 3):
            return 1
...
def readFile():
    ...
    for line in lines:
        ...
        playerTwo = ChosingYourNumber(charachters[1], playerOne)
